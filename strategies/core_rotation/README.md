...
**腿代理**: A500→沪深300(000300)、纳指=513100(QDII)、黄金=518880、债券=511260、红利/科创=对应指数
**估值口径**: 中证官网日频 `peg`（近似静态 PE）；红利股息率=全收益/价格近12m × 蛋卷 4.2% 校准；10Y=chinabond 月末；按月判手册情景→目标中枢→微调(y10<1.5 债券5%、科创PE比>7 科创0%)→归一化持有下月
**未计费**（场外 C 类年费 ~0.3-0.5%，需自行扣减）
...
**主要结论（代理口径，谨慎解读）**
- 该样本期内 红利股债收益比几乎都在 2.5 以下、红利 PE 分位高企 → 情景几乎锁定「均衡震荡」（86%）+ 阶段性「成长/牛市」（14%），**从未触发防御**；说明手册的防御阈值（比>2.5 且 PE分位<30%）在当前低利率/红利高估环境下基本不激活，情景轮动≈静态中枢；
- 因此策略 vs 静态中枢几乎无差异（年化 9.7% vs 9.6%），**未跑赢六腿等权（10.3%）**；回撤略好于基准；
- 局限：A500/科创用指数代理、纳指含 QDII 溢价/汇率、股息率为估计并单点校准、未计费、月末近似执行。完整逻辑（防御档/避险档、Forward/拥挤度微调）在样本外触发极少，本回测对它们几乎无检验力。
