...
def build_variant(close, r, buy_pos=BUY_POS, sell_pos=SELL_POS, kwin=KWIN, res=RES):
    """黄金三角: 布林空间 + MACD 动能 + KDJ 情绪 三共振抄底/逃顶。"""
    n = len(close)
...
def _plot(dates, curves, path):
    ...
    ax.set_title("510300 沪深300ETF · 黄金三角变种 vs 基础策略 vs 买入持有（净值, 起=1）")
    ax.set_ylabel("净值")
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
...
def main():
    ...
    with open(os.path.join(OUTDIR, "_v4_golden_triad_results.jsonl"), "w", encoding="utf-8") as f:
        for row in var_rows:
            f.write(json.dumps({
    ...
    with open(os.path.join(OUTDIR, "_v4_golden_triad_report.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(L))

    _plot(dates_510300, curves, os.path.join(OUTDIR, "_v4_golden_triad_510300.png"))
    print("报告:", os.path.join(OUTDIR, "_v4_golden_triad_report.md"))
    print("结果:", os.path.join(OUTDIR, "_v4_golden_triad_results.jsonl"))
